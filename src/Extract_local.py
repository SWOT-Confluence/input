...
class Extract:
    ...
    def extract_data(self, confluence_fs):
        ...
        pass_dir = Path("/home/nikki/Documents/confluence/data/sacramento")
        with scandir(pass_dir) as entries:
            date_dirs = []
            for e in entries:
                with scandir(e) as d:
                    for i in d:
                        date_dirs.append(i.name)
        ...
        for d in date_dirs:
            reach_file = glob.glob(f"{pass_dir}/*/{d}/riverobs_nominal_20201105/river_data/reaches.shp")[0]
            extract_reach_local(reach_file, reach_dict, time)
            node_file = glob.glob(f"{pass_dir}/*/{d}/riverobs_nominal_20201105/river_data/nodes.shp")[0]
...
def extract_reach_local(reach_file, reach_dict, time):
    ...
    time_step = int(Extract.TIME_DICT[Path(reach_file).parent.parent.parent.stem].strftime("%Y%m%d"))
